...
try {
    ...
    // Total Appointments
    $result = $database->query("SELECT COUNT(*) AS c FROM appointment");
    if ($result) {
    ...
    $result = $database->query("
        SELECT COUNT(*) AS c 
        FROM appointment 
        WHERE DATE(appodate) = CURDATE()
    ");
    if ($result) {
        $todayAppointments = (int)$result->fetch_assoc()['c'];
    }
    ...
    // --- Recent Appointments (Past & Today) ---
    $sql = "
        SELECT 
            a.appoid, 
            a.apponum, 
    ...
    $sql = "
        ...
        AND a.appodate <= DATE_ADD(CURDATE(), INTERVAL 7 DAY)
    ...
    $sql = "
        SELECT 
            DATE_FORMAT(appodate, '%b %Y') AS mon, 
                    ...
                    <?php foreach ($upcomingAppointments as $appt): ?>
                        <div class="appointment-item">
                            <div class="appointment-info">
                                ...
                                <p>
                                    <?php echo e($appt['service_type']); ?> • 
                                    <?php echo date('M j, Y', strtotime($appt['appodate'])); ?>
                                </p>
                            </div>
                            <div>
                                <span class="status-badge payment-<?php echo $appt['payment_status']; ?>">
